...
export function AchievementCard({ achievement }: { achievement: Achievement }) {
  return (
    <div className={`relative overflow-hidden rounded-[2rem] border p-6 transition-all duration-300 ${
      ...
      <div className="flex flex-col h-full relative z-10">
        <div className="flex justify-between items-start mb-5">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm border ${
            ...
            {renderAchievementIcon(achievement.icon, `w-7 h-7 ${achievement.unlocked ? achievement.color : 'text-zinc-500'}`)}
          </div>
          {achievement.unlocked && (
        ...
        <h3 className={`text-xl font-black tracking-tight ${achievement.unlocked ? 'text-zinc-900' : 'text-zinc-500'}`}>
          {achievement.title}
        </h3>
        <p className={`text-sm mt-1.5 flex-1 font-medium ${achievement.unlocked ? 'text-zinc-500' : 'text-zinc-400'}`}>
